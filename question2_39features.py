...
#         print(np.shape(x_filtered))
        ...
        mfcc = dct(x_filtered, type=2, axis=1, norm='ortho')[:,1:13]#[:,u2:u1]
        mfcc = np.hstack([energy,mfcc])
        delta = np.zeros((np.shape(mfcc))).astype(np.float64)
        deltadelta = np.zeros((np.shape(mfcc))).astype(np.float64)
        num_p = 2
        padmfcc = np.pad(mfcc,((num_p,num_p),(0,0)), mode='edge')#constant
        lis = np.arange(-num_p, num_p+1)
        for m in range(len(delta)):
            delta[m] = np.dot(lis, padmfcc[m:m +2*num_p+1])/(2*np.sum(np.arange(1,num_p+1)**2))
        paddelta = np.pad(delta,((num_p,num_p),(0,0)), mode='edge')#constant
        for m in range(len(deltadelta)):
            deltadelta[m] = np.dot(lis, paddelta[m:m +2*num_p+1])/(2*np.sum(np.arange(1,num_p+1)**2))
#         print(np.shape(mfcc))
#         print(np.shape(delta))
#         print(np.shape(deltadelta))
#         mfcc = mfcc[:,:12]
#         mfcc = np.hstack([mfcc,np.ones((np.shape(mfcc)[0],1))*energy])
        mfcc = np.hstack([mfcc,delta, deltadelta])
...
#         if(np.shape(mfcc)[1]!=u1):
#             mfcc = np.hstack([mfcc,np.zeros((num_filters,u1-np.shape(mfcc)[1]))])
        mfccfeatures.append(mfcc)
        labels.append(word_to_num(fol))
#         plt.show()
pickle.dump(mfccfeatures, open('mfccfeatures.pickle','wb'))
pickle.dump(labels, open('mfccfeatureslabels.pickle','wb'))
...
#             mfcc = np.hstack([mfcc,np.zeros((num_filters,u1-np.shape(mfcc)[1]))])
        ...
        labels.append(word_to_num(fol))
#         plt.show()
pickle.dump(mfccfeatures, open('mfccfeaturesvalidation.pickle','wb'))
pickle.dump(labels, open('mfccfeaturesvalidationlabels.pickle','wb'))
...
classifier = pickle.load(open("mfcc2048_2.pickle","rb"))
...
#This is for 1024 and LinearSVC and 40 par
...
import pickle
pickle.dump(classifier,open("mfcc2048_2.pickle",'wb'))
